    ...
    {active && <div className="story-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setActive(null)}>
      <article className="story-modal" role="dialog" aria-modal="true" aria-labelledby="story-modal-title">
        ...
        <div className="story-modal-content">
          ...
          <div className="story-solutions"><span>Yudaro Solutions Used</span><ul>{active.solutionsUsed.map((solution) => <li key={solution}>{solution}</li>)}</ul></div>
          <div className="story-modal-footer"><a className="text-link" href={guideLinks[active.industry]}>Read the practical industry guide <ArrowUpRight size={16}/></a><a className="button primary" href={active.cta}>Talk to Yudaro About Your Workflow <ArrowUpRight size={16}/></a></div>
        </div>
      </article>
    </div>}
  </>;
