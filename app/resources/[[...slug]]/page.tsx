...
const labels: Record<Locale, Record<string,string>> = {
  en:{resources:'Resources',intro:'Practical answers for business owners evaluating Private AI, Odoo ERP, AI + ERP integration, and workflow automation.',answer:'Direct answer',meaning:'What this means for a business',example:'Real-world example',limits:'Limitations and what to consider',questions:'Common questions',related:'Related Yudaro resources',author:'By Yudaro AI & ERP Systems',published:'Published September 5, 2026',cta:'Talk to Yudaro about your workflow',browse:'Browse practical guides',home:'Home'},
  'zh-cn':{resources:'资源中心',intro:'为正在评估私有 AI、Odoo ERP、AI + ERP 集成和工作流程自动化的企业管理者提供实用答案。',answer:'直接回答',meaning:'这对企业意味着什么',example:'实际业务示例',limits:'限制与注意事项',questions:'常见问题',related:'相关 Yudaro 资源',author:'作者：Yudaro AI & ERP Systems',published:'发布于 2026 年 9 月 5 日',cta:'与 Yudaro 讨论您的流程',browse:'浏览实用指南',home:'首页'},
  'zh-tw':{resources:'資源中心',intro:'為正在評估私有 AI、Odoo ERP、AI + ERP 整合和工作流程自動化的企業管理者提供實用答案。',answer:'直接回答',meaning:'這對企業意味著什麼',example:'實際業務示例',limits:'限制與注意事項',questions:'常見問題',related:'相關 Yudaro 資源',author:'作者：Yudaro AI & ERP Systems',published:'發布於 2026 年 9 月 5 日',cta:'與 Yudaro 討論您的流程',browse:'瀏覽實用指南',home:'首頁'},
  es:{resources:'Recursos',intro:'Respuestas prácticas para propietarios y gerentes que evalúan IA privada, Odoo ERP, integración IA + ERP y automatización.',answer:'Respuesta directa',meaning:'Qué significa para una empresa',example:'Ejemplo práctico',limits:'Limitaciones y consideraciones',questions:'Preguntas comunes',related:'Recursos relacionados de Yudaro',author:'Por Yudaro AI & ERP Systems',published:'Publicado el 5 de septiembre de 2026',cta:'Hable con Yudaro sobre su proceso',browse:'Explore guías prácticas',home:'Inicio'},
};
...
export default async function ResourcePage({params,searchParams}:{params:Promise<Params>;searchParams:Promise<Search>}){
  ...
  if(!entry){
    ...
    return <main className="resource-page"><section className="resource-hero section-shell"><span className="section-index">YUDARO / {l.resources}</span><h1>{l.resources}</h1><p>{l.intro}</p></section><section className="resource-library section-shell"><h2>{l.browse}</h2>{groups.map(group=><div className="resource-group" key={group}><h3>{group}</h3><div>{(group==='Industries' ? [...resourceEntries.filter(x=>x.pillar===group && x.slug!=='industries/restaurants'), resourceBySlug.get('industries/restaurants')!].filter(Boolean) : resourceEntries.filter(x=>x.pillar===group)).map(x=><a href={localeUrl(`/resources/${x.slug}`,locale).replace(SITE_URL,'')} key={x.slug}><strong>{x.copy[locale].title}</strong><span>{x.copy[locale].description}</span><ArrowRight size={17}/></a>)}</div></div>)}</section></main>;
  }
