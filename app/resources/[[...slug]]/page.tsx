...
export async function generateMetadata({params,searchParams}:{params:Promise<Params>;searchParams:Promise<Search>}):Promise<Metadata>{
  ...
  const metadata=pageMetadata(title,description,path); const canonical=localeUrl(path,locale); metadata.alternates={canonical};
...
export default async function ResourcePage({params,searchParams}:{params:Promise<Params>;searchParams:Promise<Search>}){
  ...
  if(!entry){
    ...
    return <main className="resource-page"><section className="resource-hero section-shell"><span className="section-index">NEXAVORIS / {l.resources}</span><h1>{l.resources}</h1><p>{l.intro}</p></section><section className="resource-library section-shell"><h2>{l.browse}</h2>{groups.map(group=><div className="resource-group" key={group}><h3>{group}</h3><div>{resourceEntries.filter(x=>x.pillar===group).map(x=><a href={`/resources/${x.slug}`} key={x.slug}><strong>{x.copy[locale].title}</strong><span>{x.copy[locale].description}</span><ArrowRight size={17}/></a>)}</div></div>)}</section></main>;
  }
  ...
  return <main className="resource-page"><article><nav className="breadcrumbs section-shell" aria-label="Breadcrumb"><a href="/">{l.home}</a><span>/</span><a href="/resources">{l.resources}</a><span>/</span><span aria-current="page">{c.title}</span></nav><header className="resource-hero section-shell"><span className="section-index">{entry.pillar}</span><h1>{c.title}</h1><p className="resource-deck">{c.description}</p><div className="resource-byline"><BookOpen size={16}/>{l.author}<CalendarDays size={16}/>{l.published}</div></header><section className="answer-block section-shell"><span>{l.answer}</span><p>{c.answer}</p></section><div className="resource-body section-shell"><section><h2>{l.meaning}</h2><p>{c.operations}</p></section><section><h2>{l.example}</h2><p>{c.example}</p></section><section><h2>{l.limits}</h2><p>{c.limits}</p></section><section><h2>{l.questions}</h2>{c.questions.map(([q,a])=><div className="resource-faq" key={q}><h3>{q}</h3><p>{a}</p></div>)}</section><aside><h2>{l.related}</h2><div className="resource-related">{entry.related.map(link=><a href={link.href} key={link.href}>{link.label}<ArrowRight size={16}/></a>)}</div></aside></div><footer className="resource-cta section-shell"><div><span className="section-index">FREE NEXAVORIS TOOL</span><h2>{l.cta}</h2></div><a className="button primary" href="/assessment">{l.cta}<ArrowRight size={16}/></a></footer></article><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/></main>;
}
