...
5. IndexNow is a future optional change-notification integration, not indexing. It requires an owned key file and deployment hook with changed canonical URLs; it was not configured without a verified operational setup. Never insert a sample production key.
